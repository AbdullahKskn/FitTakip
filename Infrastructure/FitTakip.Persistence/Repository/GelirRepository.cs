...
public class GelirRepository : IGelirRepository
{
    ...
    public async Task<decimal> TumGelirlerToplamiAsync(long IsletmeId)
    {
        return await _context.Gelirler.Where(w => w.IsletmeId == IsletmeId).SumAsync(s => s.Tutar);
    }

    public async Task<List<Gelir>> GelirleriTariheGöreGetirPaginationAsync(long IsletmeId, DateTime BaslangicTarihi, DateTime BitisTarihi, int Baslangic, int Adet)
    {
    ...
    public async Task<decimal> GelirleriTariheGöreToplaAsync(long IsletmeId, DateTime BaslangicTarihi, DateTime BitisTarihi)
    {
        var baslangicTarihi = BaslangicTarihi.Date;
        var bitisTarihi = BitisTarihi.Date.AddHours(23).AddMinutes(59).AddSeconds(59);
