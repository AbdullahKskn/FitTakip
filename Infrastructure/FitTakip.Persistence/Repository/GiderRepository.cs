...
public class GiderRepository : IGiderRepository
{
    ...
    public async Task<decimal> TumGiderlerToplamiAsync(long IsletmeId)
    {
        return await _context.Giderler.Where(w => w.IsletmeId == IsletmeId).SumAsync(s => s.Tutar);
    }

    public async Task<List<Gider>> GiderleriTariheGöreGetirPaginationAsync(long IsletmeId, DateTime BaslangicTarihi, DateTime BitisTarihi, int Baslangic, int Adet)
    {
    ...
    public async Task<decimal> GiderleriTariheGöreToplaAsync(long IsletmeId, DateTime BaslangicTarihi, DateTime BitisTarihi)
    {
        var baslangicTarihi = BaslangicTarihi.Date;
        var bitisTarihi = BitisTarihi.Date.AddHours(23).AddMinutes(59).AddSeconds(59);
